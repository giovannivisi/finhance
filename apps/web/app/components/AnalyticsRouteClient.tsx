...
export default function AnalyticsRouteClient({
  rawSearchParams,
}: {
  rawSearchParams: RawSearchParams;
}) {
  const filters = useMemo(
    () => getAnalyticsFilters(rawSearchParams),
    [rawSearchParams],
  );
  const defaultFilters = useMemo(() => getDefaultAnalyticsFilters(), []);
  const queryString = useMemo(
    () => buildAnalyticsQueryString(filters),
    [filters],
  );
  ...
  const [analytics, setAnalytics] = useState<CashflowAnalyticsResponse | null>(
    null,
  );
  ...
  if (!errorMessage && (!analytics || !accounts || !categories)) {
    return <RouteLoadingShell kicker="Analysis" title="Analytics" />;
  }
